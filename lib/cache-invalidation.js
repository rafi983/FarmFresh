...
export function invalidateAllFarmerRelatedData(queryClient, userEmail, userId) {
  console.log(
    "🔄 [Cache Invalidation] Starting comprehensive cache invalidation...",
  );
  console.log("👤 [Cache Invalidation] User identifiers:", {
    userEmail,
    userId,
  });
  ...
  console.log("🔄 [Cache Invalidation] Invalidating all dashboard queries...");
  queryClient.invalidateQueries({
    queryKey: ["dashboard"],
  ...
  if (userId || userEmail) {
    console.log(
      "🔄 [Cache Invalidation] Targeting specific user dashboard queries...",
    );
  ...
  console.log("🔄 [Cache Invalidation] Using predicate-based invalidation...");
  queryClient.invalidateQueries({
    predicate: (query) => {
  ...
  // Step 3: Force remove queries to guarantee fresh data on next request
  console.log("🔄 [Cache Invalidation] Removing cached queries...");
  ...
  // Step 4: Final fallback - clear entire cache if needed (use sparingly)
  // Only do this if we really need to ensure everything is cleared
  console.log("🔄 [Cache Invalidation] Cache invalidation completed");
}
...
export function invalidateDashboardAfterBulkUpdate(
  ...
  console.log(
    "🔄 [Bulk Update Cache] Starting dashboard-specific invalidation after bulk update...",
  );
  ...
  try {
    ...
    console.log("API service cache clear note:", error.message);
  }
  ...
  console.log("✅ [Bulk Update Cache] Dashboard invalidation completed");
}
